...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $stmt = $conn->prepare("INSERT INTO users (Username, Password, Role) VALUES (?, ?, 'Sponsor')");
        $stmt->execute([$username, $password]);
        $user_id = $conn->lastInsertId();

        $stmt2 = $conn->prepare("INSERT INTO sponsor (User_ID, First_Name, Middle_Name, Last_Name, Gender, Email,Phone, Address)
                                 VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt2->execute([$user_id, $fname, $mname, $lname, $gender, $email,$contact, $address]);

        $conn->commit();
        $message = "Sponsor registered successfully! You can now log in.";
    } catch (Exception $e) {
        $conn->rollBack();
...
<section style="padding:40px 0; text-align:center;">
  <h1 style="color:#d81b2a;">Sponsor Registration</h1>
  <div style="max-width:600px; margin:auto;">
    <form method="post" style="background:#fff; padding:25px 30px; border-radius:4px; box-shadow:0 2px 6px rgba(0,0,0,0.08); text-align:left;">
      ...
      <input name="Phone" placeholder="Contact Number" required style="width:100%;padding:10px;margin-bottom:20px;border:1px solid #ddd;border-radius:3px;">
